...
┌─────────────────────────────────────────────────────────┐
│                    .manifest.json                       │
│  Declared intent: which skills, where, from what source │
│  Edit this file to add/remove skills.                   │
├─────────────────────────────────────────────────────────┤
│                      .lock.json                         │
│  Resolved versions: pinned commit per skill             │
│  Auto-generated by `skills install` / `skills update`.  │
│  `install` trusts this — zero API calls if locked.      │
│  `update`    checks remotes — writes new lock.          │
├─────────────────────────────────────────────────────────┤
│                     ~/.agents/skills/                   │
│  ~/.codex/skills/  ~/.claude/skills/  ...               │
│  Actual files on disk. Downloaded per lock.             │
├─────────────────────────────────────────────────────────┤
│              mirrors (declared in manifest)             │
│  Derived symlinks: shared → claude, shared → opencode   │
│  Auto-created during install/update.                    │
│  Safety: never replaces real directories, never crosses │
│  declared path boundaries.                              │
└─────────────────────────────────────────────────────────┘
```

- **manifest** = 期望状态 (what you want)
- **lock**     = 已解析版本 (what you pinned)
- **disk**     = 实际安装结果 (what you have)
- **mirrors**  = namespace 派生关系 (cross-agent sharing)
